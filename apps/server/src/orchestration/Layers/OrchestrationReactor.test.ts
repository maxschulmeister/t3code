...
import { PiSessionImportReactor } from "../../provider/Layers/PiSessionImportReactor.ts";
  ...
  it("starts provider ingestion, provider command, checkpoint, and thread deletion reactors", async () => {
    ...
    runtime = ManagedRuntime.make(
      Layer.effect(OrchestrationReactor, makeOrchestrationReactor).pipe(
        ...
        Layer.provideMerge(
          Layer.succeed(PiSessionImportReactor, {
            start: () => {
              started.push("pi-session-import-reactor");
              return Effect.void;
            },
          }),
        ),
